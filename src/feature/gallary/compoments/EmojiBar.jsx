...
export const EmojiBar = ({ isVisible, emojis, onReact, onMouseEnter, onMouseLeave, userReaction }) => (
  <div 
    className={`absolute bottom-full left-4 flex gap-2 bg-black/80 backdrop-blur-md p-2 rounded-xl transition-all duration-300 transform border border-white/10 z-50 shadow-2xl ${
    ...
    {emojis.map((emoji) => {
      ...
      return (
        ...
        >
          {emoji}
          {isSelected && (
            <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-gray-500 rounded-full animate-pulse shadow-lg shadow-blue-500/50" />
          )}
        </button>
      );
    })}
